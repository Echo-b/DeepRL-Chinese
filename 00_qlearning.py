...
class QLEARNING:
    ...
    def get_max_value(self, state: np.array):
        # 找出state对应的最大的Q值
        max_value = -float("inf")
        for i in range(self.num_action):
            index = self.get_index(state, i)
...
def train(args, env, agent: QLEARNING):
    epsilon = 0.2
    ...
    for step in range(args.max_steps):
        ...
        if np.random.rand() < epsilon:
            ...
            max_value = agent.get(next_state,new_action)
        else:
            max_value = agent.get_max_value(next_state)

        target = reward + args.gamma * max_value
...
def main():
    ...
    agent = QLEARNING(num_state=args.num_state, num_action=args.num_action, low_state=low_state, high_state=high_state)
